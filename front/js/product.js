...
function saveToCard(color, quantity) {
  ...
  // condition pour voir si le canap id et coulur sont dand le kanapArray
  let ajouter = false;
  ...
  // essayez de trouver le canap et voyez s'il est en double
  const getkanap = kanapArray.find(
    (duplicateKanap) =>
      duplicateKanap.id === id && duplicateKanap.color === color
  );
